        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // GANTI BLOK KODE LOGIN LAMA ANDA DENGAN YANG INI
            ui.loginForm.addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    if (!userDoc.exists) {
                        // Jika dokumen user TIDAK ADA, buat dokumen baru sekarang juga!
                        ...
                        await userDocRef.set(newUserPayload); // Buat dokumen
                        userData = newUserPayload; // Gunakan data yang baru dibuat
                        showToast("Selamat datang! Akun Anda telah disiapkan.", "success");
                    } else {
                        // Jika dokumen sudah ada, langsung gunakan datanya
...
function renderAll() {
            ...
            async function saveMutqinScores(scores) {
                ...
                try {
                    const snapshot = await db.collection('pengaturan')
                        ...
                        .where('userId', '==', currentUserUID) // Sekarang variabelnya sudah ada
                        .get();
                    ...
                    if (!snapshot.empty) {
                        ...
                        await db.collection('pengaturan').add({
                            nama: 'skorMutqin',
                            scores: scores,
                            userId: currentUserUID // Dan di sini juga
                        });
                    }
            ...
            async function saveQuranScope(scope) {
                // BARIS INI JUGA DIPERLUKAN DI SINI
                const currentUserUID = window.appState.currentUserUID;
                if (!currentUserUID) return showToast("Error: User tidak teridentifikasi.", "error");
