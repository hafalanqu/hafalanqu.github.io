...
window.appState = {
    ...
    allStudents: [],
    allHafalan: [],
    allUsers: [], // <-- BARU: Untuk menyimpan data user (termasuk nama guru)
    quranData: [],
    pengaturan: { 
        skorMutqin: { 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const ui = {
        ...
        profile: {
            ...
            pobInput: document.getElementById('profile-pob'), // pob = place of birth
    ...
    function setupUIForRole(role) {
        ...
        // --- PERUBAHAN: Tambahkan 'profil' ke halaman yang diizinkan ---
        const siswaAllowedPages = ['profil', 'ringkasan', 'riwayat', 'quran', 'tentang'];
    ...
    function _showPageImpl(pageId) {
        ...
        // --- PERUBAHAN: Tambahkan judul untuk halaman 'profil' ---
        ...
        // --- BARU: Panggil fungsi untuk mengisi form profil saat halaman profil dibuka ---
    ...
    function showPage(pageId) {
        // --- PERUBAHAN: Tambahkan 'profil' ke halaman yang diizinkan ---
        const siswaAllowedPages = ['profil', 'ringkasan', 'riwayat', 'quran', 'tentang', 'pengaturan'];
    ...
    ui.loginForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (!userDoc.exists) {
                ...
                // --- PERUBAHAN: Tambahkan field default untuk profil ---
                const newUserPayload = {
                    email: user.email,
                    role: "guru",
                    lembagaId: "man2brebes",
                    namaLengkap: user.email.split('@')[0], // Nama default dari email
                    ttl: "", // Tempat Tanggal Lahir
                    fotoProfilUrl: "" // URL Foto Profil
                };
    ...
    function initializeAppLogic(lembagaId, uid) {
        ...
        function renderStudentProgressList() {
            ...
            // =================================================================
            // ========== PERUBAHAN LOGIKA PENGURUTAN DIMULAI DI SINI ==========
            // =================================================================

            // Hitung total skor mutqin untuk menentukan apakah ada data untuk dirangking
            const totalMutqinKeseluruhan = studentScores.reduce((sum, student) => sum + student.mutqinScore, 0);

            if (totalMutqinKeseluruhan === 0) {
                // Jika tidak ada seorang pun yang memiliki skor (belum ada setoran), urutkan berdasarkan nama
                studentScores.sort((a, b) => a.name.localeCompare(b.name));
            } else {
                // Logika pengurutan utama
                studentScores.sort((a, b) => {
                    // 1. Prioritas utama: Skor Mutqin, dari tertinggi ke terendah
                    if (b.mutqinScore !== a.mutqinScore) {
                        return b.mutqinScore - a.mutqinScore;
                    }
                    // 2. Jika Skor Mutqin sama: Total Ayat, dari terbanyak ke tersedikit
                    if (b.totalAyat !== a.totalAyat) {
                        return b.totalAyat - a.totalAyat;
                    }
                    // 3. Jika keduanya sama: Nama, secara alfabetis
                    return a.name.localeCompare(b.name);
                });
            }
            ...
            paginatedScores.forEach((student, index) => {
                ...
                // Ganti kondisi untuk menampilkan peringkat dari `totalSetoranKeseluruhan` menjadi `totalMutqinKeseluruhan`
            ...
            // ===============================================================
            // ========== PERUBAHAN LOGIKA PENGURUTAN BERAKHIR DI SINI =========
            // ===============================================================
        ...
        function renderRiwayatList() {
            ...
            // --- BARU: Buat map untuk mencari nama guru berdasarkan ID ---
            ...
            paginatedHafalan.forEach(entry => {
                ...
                // --- PERUBAHAN: Sisipkan guruNameHTML ke dalam template ---
                item.innerHTML = `
                    <div class="flex-grow">
        ...
        // --- BARU: Fungsi untuk mengisi form profil ---
        ...
        async function initApp() {
            [ui.addClassBtn, ui.addStudentSubmitBtn, ui.import.importBtn, ui.import.downloadTemplateBtn, ui.profile.saveBtn].forEach(btn => {
            ...
            ui.studentList.addEventListener('submit', async e => {
                ...
                if (e.target.classList.contains('hafalan-form')) {
                    ...
                    // --- PERUBAHAN: Tambahkan guruId saat menyimpan setoran ---
                    const newEntry = {
