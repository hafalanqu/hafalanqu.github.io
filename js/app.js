        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (togglePasswordBtn) {
                ...
                // Cek session storage untuk lembagaId juga
                const loggedInRole = sessionStorage.getItem('loggedInRole');
                const lembagaId = sessionStorage.getItem('lembagaId');
                if (loggedInRole && lembagaId) {
                    window.appState.loggedInRole = loggedInRole;
                    window.appState.lembagaId = lembagaId;
                    startApp(loggedInRole, lembagaId);
                }

                // Ubah fungsi startApp
                function startApp(role, lembagaId) {
                    ui.loginView.classList.add('hidden');
                    ui.app.classList.remove('hidden');

                    setupUIForRole(role);
                    initializeAppLogic(lembagaId); // <-- KIRIM lembagaId KE initializeAppLogic

                    // ... sisa fungsi startApp tetap sama ...
                }

            function handleLogout() {
                auth.signOut().then(() => { // Panggil fungsi signOut dari Firebase
                    // Hapus sesi dari browser
                    sessionStorage.removeItem('loggedInRole');
                    // Muat ulang halaman. Ini adalah cara paling bersih untuk reset.
                    window.location.reload();
                }).catch(error => {
                    console.error("Logout Gagal:", error);
                    // Tetap paksa reload meskipun ada error
                    window.location.reload();
                });
            }
            ...
            function startApp(role, lembagaId) { // <-- Terima 'lembagaId'
                ...
                if (initialPage && document.getElementById(`${initialPage}-page`)) {
                    ...
                    _showIconMenuImpl();
                        ...
                        // GANTI LAGI DENGAN BLOK KODE FINAL INI
            ui.loginForm.addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    if (userDoc.exists) {
                        ...
                        const lembagaId = userData.lembagaId; // <-- AMBIL lembagaId

                        if (role && lembagaId) { // <-- PASTIKAN KEDUANYA ADA
                            // Simpan role dan lembagaId di session
                            sessionStorage.setItem('loggedInRole', role);
                            sessionStorage.setItem('lembagaId', lembagaId); // <-- SIMPAN lembagaId
                            window.appState.loggedInRole = role;
                            window.appState.lembagaId = lembagaId; // <-- SIMPAN DI STATE

                            ui.loginError.classList.add('hidden');
                            startApp(role, lembagaId); // <-- KIRIM lembagaId KE startApp
                        } else {
            ...
            const onlineDB = {
                // Fungsi untuk menambah data (Kelas, Siswa, atau Hafalan)
                add(collectionName, data) {
                    // Firestore akan membuat ID unik secara otomatis
                    const dataToAdd = { ...data };
                    delete dataToAdd.id; // Hapus ID yang dibuat oleh crypto.randomUUID() jika ada
                ...
                async getAll(collectionName) {
                    ...
                    // Ubah format data dari Firestore agar sesuai dengan format yang sudah ada
                ...
                update(collectionName, data) {
                    ...
                    delete dataToUpdate.id; // ID tidak perlu disimpan lagi di dalam dokumen
            ...
            function exportAllData() {
                        ...
                        if (selectedClassId) {
                            studentsToExport = allStudents.filter(student => student.classId === selectedClassId);
                        } else {
                        ...
                        try {
                            ...
                            // --- BAGIAN BARU: Membuat file XLSX dengan SheetJS ---
                            // 1. Membuat worksheet dari data JSON (array of objects)
                            const worksheet = XLSX.utils.json_to_sheet(dataForExport);
                            // 2. Membuat workbook baru
                            const workbook = XLSX.utils.book_new();
                            // 3. Menambahkan worksheet ke workbook dengan nama "Laporan Hafalan"
                            XLSX.utils.book_append_sheet(workbook, worksheet, "Laporan Hafalan");

                            // Menentukan nama file dinamis
                            const date = new Date().toISOString().slice(0, 10);
                            let fileName = `laporan_setoran_semua_${date}.xlsx`; // Nama default
                            if (selectedClassId) {
                            ...
                            // 4. Membuat dan memicu unduhan file XLSX
            ...
            function initializeAppLogic(lembagaId) {
                ...
                const ui = {
                    ...
                    // noStudentSelected: document.getElementById('no-student-selected'), // Dihapus
                    // studentContent: document.getElementById('student-content'), // Dihapus
                ...
                function renderSurahList(surahs) {
                    ...
                    surahs.forEach(surah => {
                        ...
                        item.addEventListener('click', () => {
                            loadSurahDetail(surah.number, null); // Pass null for highlightVerseNumber on simple click
                        });
                ...
                async function loadSurahDetail(surahNumber, highlightVerseNumber = null) {
                    ...
                    try {
                        ...
                        if(response.ok) {
                            ...
                            renderDetail(surah);
                            if(!cachedData) { // Only cache if it wasn't already there
                ...
                // 1. Siapkan data template dalam format array of objects
                const templateData = [
                ...
                // 2. Buat worksheet dan workbook menggunakan SheetJS
                const worksheet = XLSX.utils.json_to_sheet(templateData);
                const workbook = XLSX.utils.book_new();
                XLSX.utils.book_append_sheet(workbook, worksheet, "Template");

                // 3. Buat dan unduh file .xlsx
                XLSX.writeFile(workbook, "template_import_siswa.xlsx");
            }

            // Ganti fungsi handleImport Anda dengan versi ini untuk notifikasi duplikat
            function handleImport(event) {
                ...
                reader.onload = async (e) => {
                    try {
                        ...
                        let skippedCount = 0; // <-- VARIABEL BARU UNTUK MENGHITUNG DUPLIKAT
                        ...
                        for (const row of jsonData) {
                            ...
                            if (!existingStudentsMap.has(studentKey)) {
                                ...
                                skippedCount++; // <-- JIKA DUPLIKAT, NAIKKAN PENGHITUNG
                            }
                        }
                        ...
                        // <-- MODIFIKASI NOTIFIKASI AKHIR
                        let message = `${newStudentsCount} siswa baru`;
                        if (newClassesCount > 0) {
                            message += ` & ${newClassesCount} kelas baru`;
                        }
...
function renderAll() {
            ...
            async function performInitialSeed() {
                try {
                    // Data contoh telah dihapus sesuai permintaan.
                    // Aplikasi akan dimulai dengan keadaan kosong.
                    console.log("Database kosong, tidak ada data awal yang ditambahkan.");
                    showToast("Assalamu'alaikum! Silakan buat kelas pertama Anda.", "info");
                } catch (e) {
                    showToast("Gagal melakukan pengaturan awal.", "error");
                    console.error("Initial setup error:", e);
                }
            }

            const MUTQIN_SCORES_KEY = 'hafalanqu_mutqin_scores';
            const QURAN_SCOPE_KEY = 'hafalanqu_quran_scope';
            const DEFAULT_MUTQIN_SCORES = {
                'sangat-lancar': 100,
                'lancar': 90,
                'cukup-lancar': 70,
                'tidak-lancar': 50,
            ...
            async function initApp() {
                            ...
                            case 'delete-class': {
                                ...
                                showConfirmModal({
                                    ...
                                    onConfirm: async () => {
                                        ...
                                        // clearStudentDetail(); // This function does not exist, safe to remove
                ...
                ui.import.fileInput.addEventListener('change', handleImport);
                 ...
                 // --- KUMPULAN EVENT LISTENER BARU UNTUK DAFTAR SISWA ---
                ui.studentList.addEventListener('click', async e => {
                    ...
                    if (deleteBtn) {
                        e.stopPropagation(); // Mencegah toggle formulir
                ...
                ui.studentList.addEventListener('change', e => {
                    // Menangani perubahan surah untuk memperbarui dropdown ayat
                ...
                ui.studentList.addEventListener('submit', async e => {
                    ...
                    if (e.target.classList.contains('hafalan-form')) {
                        ...
                        const newEntry = {
                            ...
                            catatan: '',
                            timestamp: Date.now(),
                        lembagaId: window.appState.lembagaId // <-- TAMBAHKAN INI
                        };
                        
                        await onlineDB.add('hafalan', newEntry);
                        showToast("Setoran berhasil disimpan!");
                        
                        form.closest('.hafalan-form-container').classList.add('hidden');
                    }
                });
                ...
                ui.quran.surahSearch.addEventListener('input', e => {
                    ...
                    const allSurahs = window.appState.quranData; // Use all surahs for searching
                ...
                if (ui.settings.quranScopeForm) {
                     ui.settings.quranScopeForm.addEventListener('submit', async (e) => {
                        ...
                        // Re-render student list to apply changes immediately if user goes back
                    });
                }
                 // Add listener for logout button in settings
    ...
    try {
        const commonErrorHandler = (error, collectionName) => {
            ...
            showToast(`Gagal memuat data dari ${collectionName}. Periksa izin.`, "error");
                    ...
                    // Listener untuk koleksi 'classes' dengan filter lembagaId
                    ...
                    // Listener untuk koleksi 'students' dengan filter lembagaId
                    ...
                    // Listener untuk koleksi 'hafalan' dengan filter lembagaId
